			...
			*/
			...
			if(isset($post_data[$key]['required'])) {
				...
				if($key == 'email')
					...
					if($key == 'comment')
					    $array['comment'] = $post_data[$key]['required'];
...
/*
		$array['email']  ='correo@jotmeil.com';
		$array['phone'] ='123456780';
		$array['name']    ='car';
		
*/
		
        
		$comment = rango($array['comment']);
		...
		$array['comment'] = validarEntero($array['comment']);
		...
		$array['name'] = solo_letras($array['name']);
		if($array['name'] === false) {
		    if($is_ajax === false) {
		        _redirect('#alert_mandatory');
		        exit;
...
/** ******************************* **/
	...
	function validarEntero($comment, $opciones=null){
	    if(filter_var($comment, FILTER_VALIDATE_INT, $opciones) === FALSE){
	        return false;
	    }else{
	        return true;
	    }
